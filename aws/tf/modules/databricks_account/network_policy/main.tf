...
resource "databricks_account_network_policy" "restrictive_network_policy" {
  ...
  # to the workspace is restricted to the listed IPs/CIDRs; otherwise the ingress block is omitted
  # entirely: any ingress configuration (in particular private_access) requires the account-level
  # feature "CBI Policy Private Access", and accounts without it reject the update with
  # "Feature `CBI Policy Private Access` has not been enabled for account ...".
  ...
  ingress = length(var.context_based_ingress_ip_acl) > 0 ? {
    ...
    # private access settings posture (private_access_level = "ACCOUNT"). The API populates
